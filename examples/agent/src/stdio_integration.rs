// This example shows how to use the mcp-agent crate to interact with a server that has a simple counter tool.
// The server is started by running `cargo run -p mcp-gateway` in the root of the mcp-gateway crate.
use anyhow::Result;
use mcp_agent::client::{
