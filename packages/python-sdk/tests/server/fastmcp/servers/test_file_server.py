...
def resources(mcp: FastMCP, test_dir: Path) -> FastMCP:
    ...
    @mcp_sdk.resource("file://test_dir/readme.md")
    ...
    @mcp_sdk.resource("file://test_dir/config.json")
...
async def test_list_resources(mcp: FastMCP):
    resources = await mcp_sdk.list_resources()
...
async def test_read_resource_dir(mcp: FastMCP):
    res_iter = await mcp_sdk.read_resource("dir://test_dir")
...
async def test_read_resource_file(mcp: FastMCP):
    res_iter = await mcp_sdk.read_resource("file://test_dir/example.py")
    res_list = list(res_iter)
    assert len(res_list) == 1
    res = res_list[0]
