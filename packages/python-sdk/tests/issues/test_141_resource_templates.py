...
async def test_resource_template_edge_cases():
    ...
    @mcp_sdk.resource("resource://users/{user_id}/posts/{post_id}")
    ...
    with pytest.raises(ValueError, match="Mismatch between URI parameters"):
        ...
        @mcp_sdk.resource("resource://users/{user_id}/profile")
    ...
    with pytest.raises(ValueError, match="Mismatch between URI parameters"):
        ...
        @mcp_sdk.resource("resource://users/{user_id}/profile/{section}")
        def get_user_profile_missing(user_id: str) -> str:
            return f"Profile for user {user_id}"

    # Verify valid template works
    result = await mcp_sdk.read_resource("resource://users/123/posts/456")
    ...
    with pytest.raises(ValueError, match="Unknown resource"):
        await mcp_sdk.read_resource("resource://users/123/posts")  # Missing post_id

    with pytest.raises(ValueError, match="Unknown resource"):
        await mcp_sdk.read_resource(
...
async def test_resource_template_client_interaction():
    ...
    async with client_session(mcp_sdk._mcp_server) as session:
