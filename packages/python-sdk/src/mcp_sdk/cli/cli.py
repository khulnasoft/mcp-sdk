...
try:
    from mcp_sdk.cli import claude
    from mcp_sdk.server.fastmcp_sdk.utilities.logging import get_logger
except ImportError:
    print("Error: mcp_sdk.server.fastmcp is not installed or not in PYTHONPATH")
    sys.exit(1)
...
def _build_uv_command(
    ...
    # Add mcp run command
    cmd.extend(["mcp-sdk", "run", file_spec])
    return cmd
...
def _import_server(file: Path, server_object: str | None = None):
    ...
    if not server_object:
        ...
        for name in ["mcp-sdk", "server", "app"]:
...
def version() -> None:
    ...
    try:
        version = importlib.metadata.version("mcp-sdk")
...
def dev(
    ...
    try:
        ...
        process = subprocess.run(
            [npx_cmd, "@khulnasoft/inspector"] + uv_cmd,
            check=True,
            shell=shell,
